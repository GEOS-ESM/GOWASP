...
!  ---------------------------------------------
   ...
   real(rkind1), pointer :: delp1(:,:,:) => null()
...
! 
   ...
   real(rkind1), pointer :: delp2(:,:,:) => null()
...
!
   ...
   call MAPL_AllocNodeArray(delp1,dim3,rc=ierr)
   if (ierr /= 0) call mpi_die ('main:AllocNodeArray delp1',ierr)
   dim2=(/nlon1,nlat1,1/)
...
!  Allocate space for the grid 1 fields using SHMEM
   ...
   call MAPL_AllocNodeArray(delp2,dim3,rc=ierr)
   if (ierr /= 0) call mpi_die ('main:AllocNodeArray delp2',ierr)
   dim2=(/nlon2,nlat2,1/)
...
! Note that the order here must correspond to that above
  ...
  field_name_out(5)='u'
  field_name_out(6)='v'
  field_name_out(7)='delp'
  field_name_out(8)='ozone'
  field_name_out(9)='qitot'
  field_name_out(10)='qltot'
  field_name_out(11)='ts'
!
! Read fields for grid 1   
  if (myid==1) then
    ...
    call read_shem_data (field_name(7),file_name_3d,common_path, &
                         cdatetime,3,0,delp1,ierr_read) 
  endif
!
...
! Interpolate horizontally  
  if (myid==0) then
    ...
    call intp_horz (delp1,delp2,nlevs)
  endif
!
! Wait until all processors done thus far
  call MPI_Barrier(MPI_COMM_WORLD,ierr)                 
...
! Output fields  
  if (myid == 0) then
    ...
    call write_nc4_3dfld (nlon2,nlat2,nlevs,delp2,field_name_out(7))
    u2=0.
    call write_nc4_3dfld (nlon2,nlat2,nlevs,u2,field_name_out(8))      ! ozone
    call write_nc4_3dfld (nlon2,nlat2,nlevs,u2,field_name_out(9))      ! qitot
    call write_nc4_3dfld (nlon2,nlat2,nlevs,u2,field_name_out(10))     ! qltot 
    ps2=0.
    call write_nc4_2dfld (nlon2,nlat2,  ps2(:,:,1),field_name_out(11)) ! ts
