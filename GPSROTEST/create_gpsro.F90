...
!QQQQQQ
   if (myid==ROOT) open (93,file='/discover/nobackup/rerrico/xxgg45SH.txt',form='unformatted')
   m_counter_93=0
   m_counter_all=0
!
! Read arguments
...
! Open bufr files (the input one is used to get the bufr table)
     call gpsro_rw_setup (bufr_in_file,bufr_out_file,lprint,'SIMOBS')     
...
!QQQQ       do n1=1,obs_in_tslot_num,npet
            do n1=1,obs_in_tslot_num
!QQQQ         n2=min(n1+npet-1,obs_in_tslot_num)
              n2=n1    
         do n=n1,n2
!QQQQQ           if (myid == n-n1) then 
           if (myid == ROOT) then 
             nobs=obs_in_tslot(n)
...
!
!QQQ   if (myid == ROOT ) then
!QQQ     call gpsro_obs_output (lprint,ltest_sample,cdtime_new)
!QQQ   endif 
   call MPI_Barrier(MPI_COMM_WORLD,ierr)           

   close (93)  !QQQQQQQQ
   print *,'M_COUNTER_93=',m_counter_93
