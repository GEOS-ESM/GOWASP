...
!
  subroutine setupbend (nsig,xlon,xlat,rocprof,unprof,tpdpres,zsges, &
                        tges,qges,hges,prsltmp,bend_angle,hob_out)
!
  use m_counter
...
! parameters replacing gsi use variables
  ...
  real(r_kind), parameter :: eps=rd/rv
  real(r_kind), parameter :: fv=rv/rd-one
...
! Declare passed variables
  ...
  real(r_kindx) :: hob_out      ! impactlev in terms of fraction of field lev index
  real(r_kindx) :: xlon, xlat
  real(r_kindx) :: tges(nsig)  ! tv                (bottom to top)
  real(r_kindx) :: qges(nsig)  ! specific humidity (bottom to top)
  real(r_kindx) :: hges(nsig)  ! height (m) at interface below
  real(r_kindx) :: prsltmp(nsig+1)  ! log p(cb) at interfaces
  real(r_kindx) :: zsges       ! sfc h (m)
  real(r_kindx) :: rocprof     ! local radius of curvature (m)
  real(r_kindx) :: unprof      ! geoid undulation (m)
  real(r_kindx) :: tpdpres(1)  ! impact parameter (m)
  real(r_kindx) :: bend_angle  ! bend angle output

! Declare local parameters
! Declare local variables

  logical :: lobsok, lobs_SR
  real(r_kind) sin2
  ...
  bend_angle=-1.  ! default flag
  hob_out=0.      ! default value
...
!
        if (k > nsig .and. alt <= six) then  
          grad_mod=1000.0_r_kind* &
              (nrefges(k+1,i)-nrefges(k,i))/(rges(k+1,i)-rges(k,i))
          if (abs(grad_mod)>= half*crit_grad) then
            lobs_SR=.false.
          endif
        endif
!
...
!
  ...
  do i=1,nsig
    ...
    pges(i)=1000._r_kind*exp(prsltmp(i)) ! p in Pa 
  enddo
  close (iunit)
  prsltmp(nsig+1)=log(1./1000.)  ! factor of 1000. changes from Pa to cb
!
  print ('(a)'),'Field Profile read'
  i=2
  print ('(5e15.5)'),tges(i),qges(i),hges(i),prsltmp(i)
!
!QQQQQQQ  call change_h (hges,tges,pges,prsltmp)
!
