...
!QQQQ
   ...
   real(r_kindx) :: rihob, hob, phob, logphob  
...
!QQQQ   call gpsro_ropp_plane_values_1prof (j1)  !QQQQQQ
...
!   stop
   ...
   call setupbend  (re_nsig,xlon,xlat,rocprof,unprof,tpdpres,zsges, &
   ...
   diffbend4=diffbend
!
! Compute p at level of impact parameter
   ihob=int(hob)
   rihob=real(ihob)
   logphob=(hob-rihob)*prsltmp(ihob+1)+(rihob+1.-hob)*prsltmp(ihob)
   phob=1000.*exp(logphob)  ! change p from from log(cb) to Pa
!
if (abs(xlat)<10.) print ('(a,1p4e16.5)'),'QQQ',xlat,xlon,obshght,gsibend 


!QQQ   if (abs(xlat4) > 45. ) then 
   if (xlat4 < 100. ) then  !  OUTPUT ALL 
       m_counter_93=m_counter_93+1
       write (93) m_counter_all,xlat4,xlon4,obshght4,roppbend4,gsibend4,diffbend4,phob  
   endif
! 
!
...
!
   ...
   do k=1,re_nsig
     ...
     if (m_counter_test > 3) then       ! change to values on data levels instead of interfaces
...
!  x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x
!
   subroutine gpsro_ropp_plane_values_1prof (j1)
!
...
!    
!       
   end subroutine gpsro_ropp_plane_values_1prof 
!
!
!  x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x
!
