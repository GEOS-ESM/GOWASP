...
!
   character(len=13), parameter :: obs_classes(22) = (/ 'PREPBUFR', &
      'AIRS', 'AMSUA', 'AMSUB', 'MSU', 'AMSUAAQUA', 'IASI', 'ATMS', &
      'HIRS2', 'HIRS3', 'HIRS4', 'MHS', 'GMI', 'CRIS', 'SSMIS',     &
      'GPSRO', 'SATWIND', 'GENRADTXT', 'CRISFSR', 'AVCSAM', 'AVCSPM', 'AMSR2' /)
...
! Read and check arguments
   ...
   if (argc ==  10) then 
     call GetArg( 10_4, clear_sky_suffix)
   else
...
! observations in file and finally, print out some counts of obs.
   if (myid == ROOT) then
     call pert_loop_setup (bufr_in_file,bufr_out_file, &
                           bufr_tab_file,lprint,dtype,clear_sky_suffix,ierr)
