...
!
  subroutine setupbend (nsig,xlon,xlat,rocprof,unprof,tpdpres,zsges,modify_alg, &
...
! Declare passed variables
  real(rkind1) :: hob_out      ! impactlev in terms of fraction of field lev index
  real(rkind1) :: xlon, xlat
  real(rkind1) :: tges(nsig)  ! tv                (bottom to top)
  real(rkind1) :: qges(nsig)  ! specific humidity (bottom to top)
  real(rkind1) :: hges(nsig)  ! height (m) at interface below
  real(rkind1) :: prsltmp(nsig+1)  ! log p(cb) at interfaces
  real(rkind1) :: zsges       ! sfc h (m)
  real(rkind1) :: rocprof     ! local radius of curvature (m)
  real(rkind1) :: unprof      ! geoid undulation (m)
  real(rkind1) :: tpdpres(1)  ! impact parameter (m)
  real(rkind1) :: bend_angle  ! bend angle output
...
!
! If modify, make like ROPP to define T,Q at data levels, not interfaces
        if (modify_alg > 3) then  
           qmean=qges(k)
           tmean=tges(k)
        endif
!
