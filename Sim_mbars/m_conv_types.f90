...
!
   ...
   use m_shmem_fields, only : one_point 
!
...
!
   subroutine footprint_report (dim_info,dim_fields,ctype,time1,&
                             time2,obsinfo,obs_data_o,obs_data_n,fsz)
!
!  2d- footprint obs
!
  implicit none
...
!
   ...
   real(rkind1), intent(in)     :: fsz  ! footprint size in km
...
!
   ...
   real(rkind1) :: pi
   real(rkind1) :: R  ! radius of earth
   real(rkind1) :: dy, dlat
   real(rkind1) :: dx, dlon, d
   real(rkind1) :: flat(field_jmax)
   real(rkind1) :: flon(field_imax)
   integer ::  ny,nx, iox,ioy, fi, i, j
   integer ::  miny, maxy, minx,maxx
   integer ::  xmin, xmax, ymin, ymax
   real(rkind1):: minlat, maxlat, minlon, maxlon
   integer :: npt
   integer :: fx(55000)
   integer :: fy(55000)
!
...
!
   dlon = 360./field_imax
   do i = 1,field_imax
    flon(i) = field_lon_first+(i-1)*dlon
   end do
   if (lon1 .gt. maxval(flon)) then
      lon1 = lon1-360.
   endif
   iox = floor((lon1-field_lon_first)/dlon)
!
   dlat = 180./field_jmax
   ioy = floor((lat1+90)/dlat)
   do j = 1,field_jmax
    flat(j) = -90+(j-1)*dlat
   end do
!
   ...
   minlat = -90+(miny*dlat)
   maxy = ioy+ny
   if (maxy.gt.field_jmax) then
    maxy = field_jmax
   endif
   maxlat = -90+(maxy*dlat)
!
! xrange depends on latitude
   if (ioy .le. (field_jmax/2)) then
    dx = pi*R*cos(pi*minlat/180)*dlon/180
   else
    dx = pi*R*cos(pi*maxlat/180)*dlon/180
   end if
   nx = ceiling(fsz/(2*dx))
!  print *,'lat = ',lat1, 'ioy ',ioy,'nx = ',nx, 'dx = ',dx
!
   if (nx .gt. field_imax) then
     minx = 1
     maxx = field_imax
   else
     minx = iox-nx
...
!
   do i = minx,maxx
    do j = miny,maxy
      ...
      elseif (i .gt. field_imax) then
        fi = i-field_imax
      else
        fi = i
      end if
      call gcdist(lat1,lon1,flat(j),flon(i),d)
      if (d .le. (fsz/2)) then
        npt = npt+1
        fx(npt) = fi
        fy(npt) = j
...
!
!   print *, 'time1= ',time1,' time2 = ',time2,' npt = ',npt 
   call footprint_mean(ps,fx,fy,flat,flon,rt1,time1,time2,npt)
!   print *, 'ps = ',ps,' zs= ',zs,' lat=',lat1,' lon=',lon1 
...
!
!
   end subroutine footprint_report
!
!  x x x x x x x x x x x x x xx x x x x x x x x x x x x x x x x x x x x x
!
   subroutine footprint_mean (psf,fx,fy,flat,flon,rtm,time1,time2,npt)
!
   use m_shmem_fields, only : one_point
!
   implicit none
!
   real(rkind1) :: flat(field_jmax)
   real(rkind1) :: flon(field_imax)
   real(rkind1), intent(out) :: psf  ! footprint surface pressure
   integer :: npt
   integer :: fx(55000)
   integer :: fy(55000)
   real(rkind1), intent(in)     :: rtm,time1,time2 ! braketing time for tslot
!
   real(rkind1) :: ps2t(1,2)      ! horiz interpolated ps at 2 times
   real(rkind1) :: cld2t(1,2)      ! horiz interpolated cldlow at 2 times
   real(rkind1) :: prec2t(1,2)      ! horiz interpolated prectot at 2 times
   real(rkind1) :: of2t(1,2)      ! oceanfrac
   real(rkind1) :: fps2t(1,2)      ! horiz interpolated ps at 2 times
   real(rkind1) :: seaice(1,2)      ! seaicefrac
   real(rkind1) :: ps1t(1)        ! time and horiz interpolated ps
   real(rkind1) :: cld1t(1)        ! time and horiz interpolated ps
   real(rkind1) :: t_weights(2)   ! weights used for temporal interp
   character(len=2) :: cfld 
   character(len=6) :: cldfld 
   character(len=7) :: precfld
   character(len=8) :: icefld 
   character(len=8) :: ofld
!
   integer :: i 
   logical :: raincont, icecont
   real(rkind1) :: rainthresh, rain
   real(rkind1) :: rainrand
   real(rkind1) :: pi
   real(rkind1) :: rainrate 
   real(rkind1) :: R  ! radius of earth
   real(rkind1) :: ddy 
   real(rkind1) :: ddx
   real(rkind1) :: icearea
   real(rkind1) :: dsum1, dsum2, atot, darea, landarea
!
   R = 6370.
   pi = 3.1415926
   rainthresh = 1.0
...
!
   do i = 1,npt
    ddx = R*cos(flat(fy(i))*pi/180)*2*pi/field_imax
    darea = ddx*ddy
    atot = atot+darea
    call one_point(fx(i),fy(i),cfld,ps2t)
    call one_point(fx(i),fy(i),icefld,seaice)
    call one_point(fx(i),fy(i),ofld,of2t)
    call one_point(fx(i),fy(i),cldfld,cld2t)
    call one_point(fx(i),fy(i),precfld,prec2t)
    call get_interp_time_weights (time1,rtm,time2,t_weights)
!    print *,' ofrac=',of2t(1,1),' icefrac=',seaice(1,1),',lat=',flat(fy(1)),' lon=',flon(fx(1))
!    call interp_time (1,1,1,t_weights,cld2t,cld1t)
     cld1t(1) = (cld2t(1,2)+cld2t(1,1))/2.
! calc rain rate in cloudy area in mm/hr
    if (cld1t(1).gt. 0.05) then
     rainrate = (3600*prec2t(1,2))/cld1t(1)
    else
     rainrate = 0.
    endif
    call random_seed()
    call random_number(rainrand)
    rain = rainrand*rainrate*2.0
    raincont = (rain.ge.rainthresh)
!    if (raincont) then
!      print *, 'prec=',3600*prec2t(1,2),'cld=',cld1t(1)
!    endif
    dsum1 = dsum1 + darea*ps2t(1,1)
    dsum2 = dsum2 + darea*ps2t(1,2)
    if (of2t(1,1).lt.0.99) then
      landarea = landarea+(darea*(1-of2t(1,1)))
    endif
    if (seaice(1,1).gt.0) then
      icearea = icearea+(darea*seaice(1,1))
    endif
   end do
!
!  print *, 'ice = ',icearea,', ocean=',landarea,',lat=',flat(fy(1)),' lon=',flon(fx(1))
  fps2t(1,1) = dsum1/atot
  fps2t(1,2) = dsum2/atot
    call interp_time (1,1,1,t_weights,fps2t,ps1t)
    psf=ps1t(1)
! corrupt if too much land
   if ((landarea/atot).gt. 0.05) then
     psf = psf+10000000000000000000.
   endif
! corrupt if rain
   if (raincont) then
     psf = psf+10000000000000000000.
...
! corrupt if seaice
   if (icearea/atot.gt.0.1) then
     psf = psf+10000000000000000000.
   endif
!  print *, 'psf= ',psf,'rain=',raincont,' ice = ',icearea/atot,', land=',landarea/atot,',lat=',flat(fy(1)),' lon=',flon(fx(1))
!
    end subroutine footprint_mean
!
!
!  x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x 
!
  subroutine gcdist(lat1,lon1,lat2,lon2,gcd)
!
   implicit none
!
   real(rkind1), intent(in) :: lat1    ! obs latitude 
   real(rkind1), intent(in) :: lon1    ! obs longitude 
   real(rkind1), intent(in) :: lat2    ! pt latitude 
   real(rkind1), intent(in) :: lon2    ! pt longitude 
   real(rkind1), intent(out) :: gcd    ! dist
!
!
   real(rkind1) :: R, pi ! radius of earth
