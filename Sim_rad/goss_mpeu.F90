...
!EOP ___________________________________________________________________
  ...
  character(len=*),parameter :: myname="goss_mpeu"

  integer,parameter :: Ix=1
  real*4,parameter :: R4=1.E+0
  real*8,parameter :: R8=1.D+0

  integer,parameter ::  i_kind=kind(Ix)      ! kind of default INTEGER
  integer,parameter ::  l_kind=kind(.true.)  ! kind of default INTEGER
  integer,parameter :: SP_kind=kind(R4)      ! kind of default REAL
  integer,parameter :: DP_kind=kind(R8)      ! kind of DOUBLE PRECISION real

  integer,parameter :: KI=i_kind
  integer,parameter :: KL=l_kind
  integer,parameter :: SP=SP_kind
...
subroutine stdout_open__(prefix,rewind,stat,mask,comm)
  ...
  call mype_get_(mype_world,npes_world,myname_)
  call mype_get_(mype      ,npes      ,myname_,comm=comm)
  if(present(mask)) then
    !!!! while _mype_ of _comm_ is used to check against mask, mype_world
    !!!! is always used to define the local file suffix, since it is must be
    !!!! persistent after runs to identify the output streams.

    if(and(mask,mype)/=0) return
  endif

  select case(npes_world)
  case(0:999)
    write(cmype_,'(i3.3)') mype_world
  case(1000:9999)
    write(cmype_,'(i4.4)') mype_world
  case(10000:99999)
    write(cmype_,'(i5.5)') mype_world
  case(100000:999999)
    write(cmype_,'(i6.6)') mype_world
  case default
    write(cmype_,'(i8.8)') mype_world
  endselect
...
function myid_(who)
  ...
  integer:: mype,npes
  call mype_get_(mype,npes,trim(who)//'>myid_')
  select case(npes)
  case(0:999)
    fmtid='(i3.3,2a)'
  case(1000:9999)
