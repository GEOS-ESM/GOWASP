...
!
   subroutine crtm_interface_comp_rad (prof_dim1,prof_dim2,prof_dim3, &
                 ...
                 sfc_type,btchan,btclear,clear_sky_file,ier)
!
...
! argument variables
   logical, intent(in) :: lprint
   logical, intent(in) :: clear_sky_file
...
!
   integer, intent(out) :: ier
   integer, intent(out) :: sfc_type   ! flag (1=ocean, 2=ice, 3=land, 4=snow)
   real(rkind1), intent(out) :: btchan(nchan)
   real(rkind1), intent(out) :: btclear(nchan)  ! clear-sky values if btchan is all-sky
!
! local variables
  integer :: ivegtype
...
! and earth's center and the surface and earth's center (or for CRIS, AVCSAM or AVCSPM, 
...
!
   ...
   elseif (trim(dtype) /= 'CRIS' .and. trim(dtype) /= 'CRISFSR' .and. &
           trim(dtype) /= 'AVCSAM' .and. trim(dtype) /= 'AVCSPM')  then 
!    panglr for all others except CRIS, AVCSAM and AVCSPM
...
! for CRIS,  compute panglr from relationship with lza
   if (trim(dtype) == 'CRIS' .or. trim(dtype) == 'CRISFSR' .or. &
       trim(dtype) == 'AVCSAM' .or. trim(dtype) == 'AVCSPM')  then 
...
!
! If clouds and/or precipitation are considered, then also compute clear-sky BT
   if (clear_sky_file) then
     atmosphere(1)%n_clouds=0  ! so that clouds and precip not considered now    
!
! If GMI, then specify viewing geometry differently (for set 1: chans=1-9) 
     if (trim(dtype) == 'GMI' ) then
       call crtm_interface_set_gmi (prof_dim1,prof_info,1)
...
! Re-Call CRTM for clear sky
     ...
     else  ! copy BT values (no special need for radiances here)
       do i=1,ChannelInfo(1)%n_Channels
         k=ChannelInfo(1)%Channel_Index(i)
         btclear(k)= RTSolution(k,1)%Brightness_Temperature
       enddo
     endif
!
! If GMI, then recall CRTM since channel set 2 (channels 10-13) have a 
...
!
     ...
     btclear(:)=-0.001 ! just need to be <= 0.
   endif        ! test if clear-sky values also needed
!
...
!
   ...
   real(rkind1), parameter :: default_scan_ang1=52.74
