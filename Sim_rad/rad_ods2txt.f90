...
!  
   ...
   use m_sat_info_table, only : sat_info_table_get_info_4
!
   implicit none
!
   logical :: ods_exists
...
! Get list of satellite ids for obs of requested data type
   call sat_info_table_read (sat_info_file,.true.,ierr)   
   call sat_info_table_get_info_4 (nsats_max,dtype,nsats,nchans, &
         inst_sat,said,ierr)
!
...
! Read ods files to construct list of locations and channels used
   do nsat=1,nsats
     ...
     ods_file=trim(ods_path)//'/Y'//cdtime0(1:4)//'/M'       &
              ...
              //cdtime0(9:10)//'z.ods'
!
     call ods_get_RE (.true.,trim(ods_file),ntime,nobs,ods,ierr)
     if (ierr /= 0) then
       if (ierr >= 999) then
         print *,'ods file does not exist: ierr=',ierr
         ods_exists=.false.
       else     
         print *,'ods file exists but read problems detected: ierr=',ierr
         stop
       endif   
     else
       ods_exists=.true.
