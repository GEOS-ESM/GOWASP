...
! Read arguments
   ...
   if (lprint .and. argc /= 6) then
     print *,' usage must be: prog.x dtype cdtime_ref', & 
   ...
   call GetArg( 4_4, bufr_in_file)
   call GetArg( 5_4, bufr_out_file)
...
!  Read phis (cdtime_old not used here)
   ...
   call time_computer_new_cdtime (cdtime_ref,cdtime1,rtime1,ierr)
   call gpsro_fields_read (myid,cdtime_ref,cdtime1,'PHIS',ierr_read) 
   if (ierr_read /= 0) then
...
!       call gpsro_fields_read (myid,cdtime1,cdtime2,'PTQ',ierr_read)
       call gpsro_fields_read (myid,cdtime_ref,cdtime2,'PTQ',ierr_read)
       if (ierr_read /= 0) then
         call mpi_die (my_name,ierr_read)
       endif 
       call MPI_Barrier(MPI_COMM_WORLD,ierr) 
