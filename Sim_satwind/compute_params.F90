  program compute_amv_sim_params
!
! Program to determine parameters of probability functions to be used for
! determining if an AMV obs is present.  Input is the output from the program
! count_amv.f90. Output from the program here is then used as input to the
! program create_amv.F90.
!
...
! Variables concerning time keeping: 
   integer :: ntime_periods      ! num of ana or BUFR data periods to process
   integer :: ntime_ana          ! count of analysis period
   integer :: ntime_nr           ! count of nr t spans in current ana period
   integer :: ntime_kx           ! count of interp times in current ana period
...
!
   real(rkindh) :: dhours_ana    ! t between analysis (or BUFR file) periods 
   real(rkindh) :: dhours_nr     ! time between nr data files used
   real(rkindh) :: dhours_kx     ! spacing between (possibly) interpolated t
   real(rkindh) :: thours_ana    ! ana t period hours from first time 
   real(rkindh) :: thours_kx     ! current t rel to beginning time
   real(rkindh) :: rhours_nr     ! nr data time relative to ana time
   real(rkindh) :: rhours_nr_next  ! t to check if nr t to be incremented 
   real(rkindh) :: rhours_kx     ! interpolated field time relative to ana time
   real(rkindh) :: time0_offset  ! first t in current ana period rel to ana t
   real(rkind1) :: tnorm         ! 
   ...
   real(rkind1) :: time1_nr(6)   ! nr time at start of current interp period 
   real(rkind1) :: time2_nr(6)   ! nr time at end of current interp period 
   real(rkind1) :: time0_kx(6)   ! t for 1st interp field in this run
   real(rkind1) :: time1_kx(6)   ! t for 1st interp field in current ana period
...
!
   ...
   character(len=8)  :: ciseed
   character(len=240) :: field_list_file   ! rc file of NR field info
   character(len=240) :: histogram_file    ! file of field histograms
   character(len=240) :: kx_table_file_in  ! file of obs type imfo and counts
   character(len=240) :: kx_table_file_out ! file of _in plus prob paramaters
!
...
!
   ...
   real(rkind1), allocatable :: ipwg_max(:,:,:)
...
! Read arguments defined when program is executed in script
   argc = iargc()
   if (lprint .and. argc /= 6) then
     print *,' usage must be: compute_params.x field_list_file', &
             ' kx_table_file_in kx_table_file_out histogram_file', &
             ' ciseed ctest'
     call mpi_die (my_name,77)
   endif
!
   call GetArg( 1_4, field_list_file)
   call GetArg( 2_4, kx_table_file_in)
   call GetArg( 3_4, kx_table_file_out)
   call GetArg( 4_4, histogram_file)
   call GetArg( 5_4, ciseed)
   call GetArg( 6_4, ctest)
!
! Set some variables based on the arguments 
   read (ciseed,'(i8)') iseed
!
   if (ctest == 'T') then
     ltest=.true.
     ltest_1time=.true.
...
! (...,5) here means only read first 5 parts of table file
   call kx_table_read (kx_table_file_in,lprint,5,ierr)
   if (ierr /= 0) then
     print *,'Error detected in call to kx_table_read: ierr=',ierr 
     call mpi_die (my_name,ierr)
   endif      
   if (ierr /= 0) call mpi_die (my_name,ierr)
...
!
   ntime_periods=kx_times_params
   cdtime0_ana=kx_cdtime0_params
...
! Allocate arrays of profiles at a single lat,lon
   ...
   allocate (ipwg_max(kx_ipw_nlevs,kx_num,2))
...
! (need to use cdtime1_kx on next line if this is to match older software)
...
! Re set NR data times that sandwich the time for the interpolated field 
       if (rhours_kx > rhours_nr_next-0.001_rkindh) then ! move to next nr time slot
         ntime_nr=ntime_nr+1 
         ...
         call time_compute_pack (time1_nr,cdtime1_nr)  ! new begin time of slot) 
         call time_compute_pack (time2_nr,cdtime2_nr)  ! new end time of slot)
       endif
       nband_time=mod(ntime_kx,kx_jbins_time)
!
       if (ltest .and. lprint) then 
         print *,' '
         print ('(a,i3,2x,a)'),'Processing for field time: ', &
               ntime_kx,cdtime2_kx
...
!
! If testing, print out table of max values of q for each lat band and p layer
       if (ltest .and. ntime_count==1 .and. myid == 0) then
         call amv_fields_compute_q_table (nlons,nlats)
       endif
       call MPI_Barrier(MPI_COMM_WORLD,ierr)
!
...
! Loop over lat (exclude poles)
       do j=2,nlats-1
         if (mod(j-2,npet) == myid) then  ! distribute onto different processors
           lat=-90.+(j-1)*dlat
...
! Also vertical interpolate to create profiles if requested
...
!
! Look for obs for each kx separately
             do k=1,kx_num
...
!
! Increment histogram for VIS/IR obs based on cloud and wind profile
                   call amv_accum_bins (ncldmax,ncld,k,jbin,  &
                                      cld_obsc,cld_puvs,cld_frac)
!
                   ...
                   call amv_accum_bins (kx_ipw_nlevs,kx_ipw_nlevs,k,jbin, &
                                        ipw_obsc,ipw_puvs,prof_ipwg(:,1))
!
! Compute maximum of gradient for this type and each ipw p-range for diagnostic
                   do nipw=1,kx_ipw_nlevs
                     ipwg_max(nipw,k,1)= &
                                max(ipwg_max(nipw,k,1),prof_ipwg(nipw,2))
                   enddo
                 endif
!
               endif   ! check on kx_present
             enddo     ! loop over k (obs types)
!
           enddo       ! i
         endif         ! myid 
       enddo           ! j
...
!
   call MPI_Barrier(MPI_COMM_WORLD,ierr)
!
! For myid=0, get sum of values of kx_histogram(:,:,:,:,1) residing on
! all processors. Place the sum in array kx_histogram(:,:,:,:,2)
   i=kx_cbins*kx_pbins*kx_jbins*kx_num  ! size of kx_histogram(:,:,:,:,1)
   call MPI_ALLreduce (kx_histogram(:,:,:,:,1),kx_histogram(:,:,:,:,2),i, &
                       MPI_INT,MPI_SUM,MPI_COMM_WORLD,ierr)
   call MPI_Barrier(MPI_COMM_WORLD,ierr)
!
! Do the same for kx_pt_count
   i=kx_jbins*kx_num  ! size of kx_pt_count(:,:,1)                                    
   call MPI_ALLreduce (kx_pt_count(:,:,1),kx_pt_count(:,:,2),i, &
                       MPI_INT,MPI_SUM,MPI_COMM_WORLD,ierr)
   call MPI_Barrier(MPI_COMM_WORLD,ierr)
!
! Do the same for ipwg_max
   i=kx_ipw_nlevs*kx_num
   call MPI_ALLreduce (ipwg_max(:,:,1),ipwg_max(:,:,2),i, &
                       MPI_FLOAT,MPI_SUM,MPI_COMM_WORLD,ierr)
   call MPI_Barrier(MPI_COMM_WORLD,ierr)
!
! Compute parameters from histogram
   if (myid == 0) then
     tnorm=real(ntime_periods)
     call amv_normalize_counts (tnorm)
     kx_obs_count(:,:,:,2)=kx_obs_count(:,:,:,1)
     do k=1,kx_num
       call amv_tune_params (kx_ifunc(k),k)
     enddo
     call kx_table_write (kx_table_file_out)
!
     if (trim(histogram_file) /= 'none') then
       call kx_histogram_print (histogram_file)
     endif
!
! Print maximum difference between max and min values in target regions for
! the ipw ranges.
     print *,' '
     ipwg_max(:,:,1)=ipwg_max(:,:,2)/npet
     print *,'Max difference between ipw in target areas for ipw ranges:'
     print *,'(as averaged over all processors)'
     print *,' k scale1 maxdiff1 scale2 maxdiff2 ...'
     do k=1,kx_num
       i=kx_ipw_ids(k)
       print ('(i3,5(2x,f6.2,f8.3))'),k, &
           (kx_ipw_bparm(1,nipw,i),ipwg_max(nipw,k,1),nipw=1,kx_ipw_nlevs)
     enddo
!
! Print average counts of numbers of locations considered
     print *,' '
     print *,'Average counts of numbers of locations considered'
     print *,'k, kx_pt_counts(1:N/2,k,2), next line kx_pt_counts(N/2:N,k,2)'
     do k=1,kx_num
       j=(kx_jbins+1)/2
       print ('(i3,10i8)'),k,kx_pt_count(1:j,k,2)                                    
       print ('(3x,10i8)'),kx_pt_count(j+1:kx_jbins,k,2) 
     enddo                                   
!
   endif  ! end of test on myid
!
   call MPI_Barrier(MPI_COMM_WORLD,ierr)
!
   if (ltestgrid) then
...
!
     ...
     deallocate (x_random,prof_ipwg,ipw_puvs,ipw_obsc,ipwg_max)
!
...
!  x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x x
!
   end program compute_amv_sim_params
