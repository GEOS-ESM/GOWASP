...
!
    ...
    use m_obs_error_table, only : et_corr_shapes
...
!
    ...
    real(rkind1), allocatable :: fits(:,:,:)  ! params for fit of added correl 
!
...
!
    print *,' '
    print *,'BEGIN PROGRAM estimate_eparams_rad_hcorr'
!
! Read arguments
    argc = iargc()
    if (argc .ne. 8) then
...
! from hcorr_tables
      ...
      if (ierr == 0) then
        ...
        print *,'program stooping here (1)'
        stop
      endif 
    endif
    plevs_t(:)=0.  ! no plevs defined for radiances
!
...
! from hcorr_tables
      ...
      if (ierr == 0) then
        ...
        print *,'program stooping here (2)'
        stop
      endif 
    endif
    plevs_o(:)=0.  ! no plevs defined for radiances
!
...
!
    enddo
!
! Compute difference in binned covariances if available. Also, assign 
! the min of target and osse mean numbers of obs per DAS cycle as a 
! measure of statistical reliability (larger measue means more reliable).
...
! represents what the difference would be if no added were added.
    if (lcorr_both) then
      do k=1,nk
        C_d(1,k,1)=C_t(1,k,1)-C_o(1,k,1)+pert_fac2*Rvar_o(k)
        do n=2,nbins
