...
set path_target_stats=${STATS_PATH}/${target_dir}/Corr_rad_June_5d
set path_osse_stats=${STATS_PATH}/${osse_dir}/Corr_rad_June_5d

# set filesin to "hcor*.bin" if all data types within input directory 
# to be treated the same; otherwise set to, e.g., "hcor*airs*bin" for 
# a particular obs type. 
# The " " is necessary in the set statement when * is in the generic name  
set filesin="hcor*.bin" 

# NOTE THAT the present code does nopt work right for AVHRR since it gets confused by
# the names AVHRRODS and AVCSPM/AM that occurrs because of using avhrr3 ins some files
# and avhrr in others (See sat_info.rc) 
...
cp $PROGHOME/eparams_rad_hcorr.x prog.x
...
set file_target_stats=${path_target_stats}/hcor_${target_exp}_${instr}_${sat}_${tail}

set file_osse_stats=${path_osse_stats}/hcor_${osse_exp}_${instr}_${sat}_${tail}

set file_output=${OUTPUT_PATH}/est_${osse_exp}_${instr}_${sat}.txt

if (-e $file_target_stats) then  # then both stats files should exist
./prog.x $instr $sat $error_rc $file_target_stats $file_osse_stats $sat_info_file $file_output $corr_func_new >> logfile.txt
endif
