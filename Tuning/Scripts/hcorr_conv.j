...
##SBATCH --qos=debug
#
# Execute hcorr_sumods.x and hcorr_tables.x to create files of tables of 
# means, standard deviations, and horizontal correlations of O-F for conv 
# obs for a given region. 

# requires 3.5 hrs/month
...
@ ntimes_other =   20       # number of synoptic times to process            
@ ntimes_raobs =   10      # number of synoptic times to process for raobs  
set region="GLOB"           # geo region to compute correls (see below)      

# Set the output directory
setenv OUTPUT /discover/nobackup/projects/gmao/nwposse/develop/ODSstats/${exp_dir}/Corr_conv_June_5d
...
setenv RESTART_DIR /discover/nobackup/projects/gmao/nwposse/develop/ODSstats/${exp_dir}/Corr_conv_July/Save_sums
setenv RESTART_I  0  # set to 0 unless this is a restart
setenv RESTART_I_RAOB 0  # half the value for Raobs

# Set kx of obs to process
set kx_set= (120 130 131 132 133 180 181 182 187 220 221 223 224 229 230 231 232 233 242 243 244 245 246 247 250 252 253 254 257 258 259 280 282 290)  
#set kx_set= (242 243 244 245 246 247 250 252 253 254 257 258 259 280 282 290)  
#set kx_set= (120 130 131 132 133 180 181 182 187 220 221 223 224 229 230 231 232 233)  
#
...
foreach kx ($kx_set)
  ...
  if ($kx == "120" || $kx == "220") then 
    ...
    set delp=$delp_r
    set RESTART_X=$RESTART_I_RAOB
  else
    set addhours="$addhours_other"
    set ntimes="$ntimes_other"
    set delp=$delp_o
    set RESTART_X=$RESTART_I
  endif 
...
# type in the times previously considered).
    if ($RESTART_X == 0) then
      ...
      cp $RESTART_DIR/save_${output_file}_${region}_$RESTART_X.bin input_stats
