...
set eof_file=$GOWASP_PATH/Tuning/Scripts/impacts_eof.txt # file of EOF mark to add to end
...
set addhours="+24"          # hours between times to consider (e.g., "+24"  
set region="GLOB"           # geo region to compute correls (see below)    
set sfc_type="BOTH"         # SEA, LAND, or BOTH
set impact_metric="imp3_twe"

set kx_set= (amsua_n15 amsua_n18 amsua_n19 amsua_aqua amsua_metop-a amsua_metop-b hirs4_metop-a hirs4_metop-b mhs_n18 mhs_metop-a mhs_metop-b airs_aqua iasi_metop-a iasi_metop-b cris_npp atms_npp atms_n20 ssmis_f17)
...
cp $PROGHOME/impacts_rad.x prog.x
...
foreach instrument ($kx_set)
  ...
  set sat_inst=`echo $instrument | cut -d_ -f1`  # extract string before "_" denoting obs instument 
  set platform=`echo $instrument | cut -d_ -f2`  # extract string after "_" denoting obs satellite platform 
...
# loop over times 
  while ( $i <= $ntimes ) 
    ...
    @ j = $j + 1
    ./prog.x ${yyyy3}${mm3}${dd3} ${hh3}0000 ${latN} ${latS} ${lonW} ${lonE} $sfc_type ${sat_info_file} $sat_inst $platform ${file_name} >> impact_printout
...
echo "${j} files processed"
echo "${i_no} = number of files not existing"

if (! -e $OUTPUT ) mkdir -p $OUTPUT
/bin/mv impact_printout ${OUTPUT}/impacts_rad_printout
